...
(function ($, window, console, undefined) {
	...
	FooBox.browser = {
		...
		check: function () {
			...
			FooBox.browser.isChrome = navigator.appVersion.indexOf("Chrome") > -1;
			...
			if (FooBox.browser.isSafari) {
				...
				if (FooBox.browser.iOS == true) { FooBox.browser.css = 'fbx-ios'; }
				FooBox.browser.Mac = /(Mac)/g.test(navigator.platform);
				if (FooBox.browser.Mac == true) { FooBox.browser.css = 'fbx-mac'; }
			} else if (FooBox.browser.isIE) {
	...
	FooBox.Modal = function (instance) {
		...
		this.setup = {
			...
			options: function (options) {
				...
				if ((FooBox.browser.isIE && FooBox.browser.version < 9) || (FooBox.browser.isSafari && FooBox.browser.iOS)) {
		...
		this.show = function (first) {
			...
			if (_this.FooBox.options.hideScrollbars) { $('body').css('overflow', 'hidden'); }
			...
			if (!_this.element.is(':visible')) {
				...
				_this.element
					.find('.fbx-inner').hide().css({ 'width': '100px', 'height': '100px', 'margin-top': '-50px', 'margin-left': '-50px' })
					.end()
					.show();
			...
			if (blevt.fb.handled == true) { return; }
					...
					_this.transitionOut(current, function () {
						...
						_this.resize(size, function () {
							...
							_this.transitionIn(next, function () {
								...
								if (item.overflow == true) { next.add(current).css('overflow', ''); }
								else { next.add(current).css('overflow', 'hidden'); }
								current.empty();
		...
		this.resize = function (size, success, error) {
			try {
				...
				var item = _this.FooBox.items.current();
				_this.FooBox.raise('foobox.beforeResize', { 'item': item });

				var $inner = _this.element.find('.fbx-inner'),
					$spacer = _this.element.find('.fbx-inner-spacer'),
					buttons = _this.FooBox.options.showButtons && _this.FooBox.items.multiple();
				...
				var mpt = parseInt($spacer.css('padding-top'), 0),
					...
					ch = parseInt($inner.css('height'), 0),
					cw = parseInt($inner.css('width'), 0),
					vdiff = (buttons ? mpt + mpb : mpt * 2) + (padding * 2) + ibt + ibb,
					hdiff = mpl + mpr + (padding * 2) + ibl + ibr,
					mh = _this.element.height() - vdiff,
					mw = _this.element.width() - hdiff,
					ratio = mw / size.width;

				// If this is a portrait calculate the ratio using the height variables instead of the width.
				if ((size.height * ratio) > mh) { ratio = mh / size.height; }

				var ih = size.height,
					iw = size.width;

				// Calculate the size to scale the modal to if fitToScreen is set to true.
				if (_this.FooBox.options.fitToScreen === true || (item.proportion == true && (size.height > mh || size.width > mw))) {
					ih = Math.floor(size.height * ratio);
					iw = Math.floor(size.width * ratio);
				} else if (item.proportion == false){
					if (size.height > mh){ ih = mh; }
					if (size.width > mw){ iw = mw; }
				}
				...
				function finish(width, height) {
					if ((item.overflow == true && (width < (item.width * 0.8) || height < (item.height * 0.8)))
						|| (item.overflow == true && item.proportion == false && (width < item.width || height < item.height))) {
						_this.element.find('.fbx-item').css({ 'width': item.width, 'height': item.height });
					} else {
						_this.element.find('.fbx-item').css({ 'width': '', 'height': '' });
					}
					var s = new FooBox.Size(width, height);
					_this.FooBox.raise('foobox.afterResize', { 'item': item, 'size': s });
					if ($.isFunction(success)) { success(s); }
				}

				if (ih !== ch || iw !== cw) { // If the size has changed resize it.
					var mt = -((ih / 2) + padding + ((ibt + ibb) / 2) + (buttons ? mpb - mpt : 0)),
						ml = -((iw / 2) + padding + ((ibl + ibr) / 2));
		...
		this.close = function () {
			_this.element.hide();
			if (_this.FooBox.options.hideScrollbars) { $('body').css('overflow', ''); }
		...
		this.items = {
			...
			rel: function (element) {
				...
				var rel = element.attr('rel'),
					len = typeof _this.options.rel == 'string' ? _this.options.rel.length : 0;
	...
	FooBox.open = function (arg) {
		if ($('body').hasClass('fbx-active')){ return null ; }
		var fbx = null, i;
	...
	FooBox.resize = function(width, height){
		if (width || height){
			...
			for (i = 0; i < FooBox.instances.length; i++){
				fbx = FooBox.instances[i];
				if (fbx == null || !(fbx.modal instanceof FooBox.Modal) || !FooBox.isjQuery(fbx.modal.element) || !fbx.modal.element.is(':visible')){ continue; }
...
(function ($, FooBox) {
	...
	FooBox.Captions = function (instance) {
		...
		this.checkHeight = function () {
			...
			if (!_this.FooBox.options.captions.enabled || !item.captions) { return; }
			var $caption = modal.find('.fbx-caption'),
				$desc = $caption.find('.fbx-caption-desc');
...
(function ($, FooBox) {
	...
	FooBox.DeepLinking = function (instance) {
			...
			parseItem: function(e){
				...
				switch(e.fb.item.type){
					case 'image':
						...
						break;
					case 'html':
						e.fb.item.deeplink = e.fb.item.selector.replace('#', '');
						break;
					case 'video':
		...
		this.hash = {
			 ...
			 */
			get: function () {
				if (location.hash.indexOf('#' + _this.FooBox.options.deeplinking.prefix) === -1) { return null; }
	...
	FooBox.Keyboard = function (instance) {
		...
		this.handlers = {
			...
			unbind: function(all){
				...
				$(document).unbind('keydown.foobox', _this.handlers.onKeydown);
			},
			/**
			 * Handles the foobox.initialized event binding the various events needed by this addon to function.
			 * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
			...
			initialized: function () {
				_this.handlers.unbind();
				$(document).bind('keydown.foobox', _this.handlers.onKeydown);
			},
			/**
			...
			onKeydown: function (e) {
				...
				if (!modal.element.is(':visible')) { return true; }
				var key = e.which;
...
(function ($, FooBox) {
	...
	FooBox.BPInfo = function (breakpoints) {
		this.width = window.innerWidth || document.documentElement.clientWidth || (document.body ? document.body.offsetWidth : 0);
		this.height = window.innerHeight || document.documentElement.clientHeight || (document.body ? document.body.offsetHeight : 0);
		...
		this.handlers = {
			...
			resize: function () {
				_this.timers.resize.start(function () {
					_this.style();
					if (!_this.FooBox.modal.element.is(':visible')) { return; }
					var item = _this.FooBox.items.current(),
					...
					for (var i = 0; i < e.fb.options.social.links.length; i++) {
						...
						if ($button.length > 0) {
							...
							if ($.inArray(e.fb.item.type, excludes) !== -1) {
								...
								if (url != '' && url != '#') {
									$button.attr('href', _this.cleanUrl(url)).show();
								} else {
									$button.removeAttr('href').hide();
								}
							}
		...
		this.handlers = {
			...
			onCreateCaption: function (e) {
				...
				if (cont.hasClass('gallery')) {
					if (opt.captions.overrideTitle === false) { title = el.parents(".gallery-item:first").find(".wp-caption-text:first").html() || ''; }
					if (opt.captions.overrideDesc === false) { desc = el.find('img').attr('alt') || ''; }
				} else if (cont.hasClass('wp-caption')) {
					if (opt.captions.overrideTitle === false) { title = el.find('img').attr('title') || ''; }
					if (opt.captions.overrideDesc === false) { desc = el.parents(".wp-caption:first").find(".wp-caption-text:first").html() || ''; }
				} else if (cont.hasClass('tiled-gallery')) {
					if (opt.captions.overrideTitle === false) { title = el.parents(".tiled-gallery-item:first").find(".tiled-gallery-caption").html() || el.find('img').data('image-title') || el.find('img').attr('title') || ''; }
					if (opt.captions.overrideDesc === false) {
						desc = $.trim(el.find('img').data('image-description')) || '';
						desc = desc.replace(/(^<p>)|(<\/p>$)/ig, '');	//replace opening and closing P tags that are generated by tiled gallery
					}
				} else {
					return;
				}

				if (title && $.trim(title) == $.trim(desc)) desc = null;
	...
	FooBox.HtmlHandler = function(instance) {
		...
		this.handles = function (element) {
			var selector = _this.FooBox.options.html.findSelector(_this.FooBox, element),
				handle = $(element).attr('target') === 'foobox' && typeof selector === 'string' && selector.match(_this.regex) != null && ($(selector).length > 0 || $(element).data('ajax') == true);
...
(function ($, FooBox) {
	...
	/** @type {Object} - An object containing two flags indicating whether or not the values were set automatically. */
	FooBox.Item.prototype.auto = { width: false, height: false };
	...
	FooBox.IframeHandler = function (instance) {
		...
		/** @type {RegExp} - The regular expression used to check whether or not this handler handles an item. */
		this.regex = /^https?/i;
		/** @type {RegExp} - The regular expression used to check for videos. */
		this.videoRegex = /(youtube(-nocookie)?\.com\/(watch|v|embed)|youtu\.be|vimeo\.com)/i;
		...
		this.handles = function (element) {
			...
			var handle = $(element).attr('target') === 'foobox' && typeof href === 'string' && href.match(_this.regex) != null && !href.match(_this.videoRegex);
		 ...
		 */
		this.getSize = function (item) {
			if (item.auto || item.width == null || isNaN(item.width) || item.width == 0 || item.height == null || isNaN(item.height) || item.height == 0){
				item.auto = item.auto || { width: false, height: false };
				if (item.auto.width == true || item.width == null || isNaN(item.width) || item.width == 0){
				...
				if (item.auto.height == true || item.height == null || isNaN(item.height) || item.height == 0){
...
(function ($, FooBox) {
	...
	FooBox.VideoHandler = function (instance) {
		...
		this.type = 'video';
		/** @type {RegExp} - The regular expression used to check whether or not this handler handles an item. */
		this.regex = /(youtube(-nocookie)?\.com\/(watch|v|embed)|youtu\.be|vimeo\.com)/i;
